...
function createTemperatureHumidityChart() {
  const ctx = document
...
function createWindSpeedChart() {
  ...
  new Chart(ctx, {
    type: "line",
    data: {
      labels: myForecastData.map((item) => `${item.date} ${item.time}`),
      datasets: [
        {
          label: "Wind Speed",
          data: myForecastData.map((item) => item.windSpeed),
